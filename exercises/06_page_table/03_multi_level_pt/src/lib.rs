...
impl Sv39PageTable {
    ...
    /// 在 ppn 对应的页表中取出第 vpn 个页表项
    fn get_pte(&self, ppn: u64, vpn: usize) -> Option<u64> {
        if let Some(tb) = self.nodes.get(&ppn) {
            Some(tb.entries[vpn])
        } else {
            None
        }
    ...
    fn get_pte_mut(&mut self, ppn: u64, vpn: usize) -> Option<&mut u64> {
        if let Some(tb) = self.nodes.get_mut(&ppn) {
            Some(&mut tb.entries[vpn])
        } else {
            None
        }
    }
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (0..3).rev() {
            ...
            if level == 0 {
                *self.get_pte_mut(cur_ppn, vpn).unwrap() = (pa >> 12 << 10) | flags;
            } else {
                if self.get_pte(cur_ppn, vpn).unwrap() & PTE_V == 0 {
                    *self.get_pte_mut(cur_ppn, vpn).unwrap() = (self.next_ppn << 10) | 1;
                    self.nodes.insert(self.next_ppn, PageTableNode::new());
                    self.next_ppn += 1;
                }

                cur_ppn = self.get_pte(cur_ppn, vpn).unwrap() >> 10;
            }
        }
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..3).rev() {
            let vpn = Self::extract_vpn(va, level);
            let pte = self.get_pte(cur_ppn, vpn).unwrap();
            if pte & PTE_V == 0 {
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                let ppn = pte >> 10;
                let shift = if level == 0 { 12 } else { 21 };
                let pa = (ppn << 12) | (va & ((1 << shift) - 1));
