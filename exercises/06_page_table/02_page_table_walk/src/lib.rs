...
impl SingleLevelPageTable {
    ...
    pub fn map(&mut self, vpn: usize, ppn: u32, flags: u8) {
        self.entries[vpn].replace(PageTableEntry { ppn, flags });
    }
    ...
    pub fn translate(&self, va: u32, is_write: bool) -> TranslateResult {
        ...
        match self.lookup(vpn) {
            Some(pte) if (pte.flags & PTE_VALID) == 0 => TranslateResult::PageFault,
            Some(pte) if is_write && (pte.flags & PTE_WRITE) == 0 => TranslateResult::PermissionDenied,
            Some(pte) => TranslateResult::Ok(make_pa(pte.ppn, offset)),
